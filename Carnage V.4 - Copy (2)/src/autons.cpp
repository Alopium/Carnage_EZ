...
void default_constants() {
  // P, I, D, and Start I
  chassis.pid_drive_constants_set(28, 0.10, 295);         // Fwd/rev constants, used for odom and non odom motions
  ...
  chassis.pid_odom_angular_constants_set(10, 0, 72 );    // Angular control for odom motions
...
//clamp 1
  ...
  pros::delay(500);
  setIntake(-127); 
  chassis.pid_odom_set({{20,-29}, fwd, 70});
  chassis.pid_wait();
...
// clamp 2 
  setIntake(0); 
  chassis.pid_odom_set({{2, -51.5},fwd, 70});
  chassis.pid_wait();
  chassis.pid_odom_set({{-19,-51.5},rev, 50});
  chassis.pid_wait();
  clamps.set(false);
  pros::delay(500);
  setIntake(-127); 
